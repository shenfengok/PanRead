  ...
  export default {
    ...
    methods :{
      ...
      getlist (is_next) {
        ...
        this.axios.get('/api/jing.php?do=list&prefix='+this.$route.query.prefix+'&pid=' + this.$route.query.id + '&start=' + this.cur)
          .then((res) => {
            if (res.data) {
              this.list = res.data
            } else {
      ...
      router2(subId){
        this.$router.push({ path: '/jingdtl', query: {  id:this.$route.query.id,subId:subId,pname:this.$route.query.title,title:this.$route.query.title,prefix:this.$route.query.prefix }})
      },
      back(){
