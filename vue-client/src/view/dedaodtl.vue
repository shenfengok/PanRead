    ...
    methods :{
        ...
        this.axios.get('/api/'+this.$route.query.prefix+'/dtl?id=' + this.$route.query.id + '&cur=' + cur)
          .then((res) => {
            if (res.data) {
              ...
              this.getcnt(this.findContentUrl(finfo),this.findAudioPath(finfo));
              this.loghis(cur);
            } else {
              console.log(res)
            }
      ...
      findContentUrl(file_info){
        for(let i=0; i<file_info.length; i++) {
          if(file_info[i].path.endsWith('.html')){
            return file_info[i].path;
          }
        }

        for(let i=0; i<file_info.length; i++) {
          if(file_info[i].path.endsWith('.pdf')){
            return file_info[i].path;
          }
        }
        for(let i=0; i<file_info.length; i++) {
          if(file_info[i].path.endsWith('.png')){
            return file_info[i].path;
          }
        }
      },
      findAudioPath(file_info){
        for(let i=0; i<file_info.length; i++) {
          if(file_info[i].path.endsWith('.m4a')){
            return file_info[i].path;
          }
        }

        for(let i=0; i<file_info.length; i++) {
          if(file_info[i].path.endsWith('.mp3')){
            return file_info[i].path;
          }
        }
      ...
      getcnt(url,audio){
        this.axios.get('/pan/'+this.$route.query.prefix+'-all' + url)
          .then((res) => {
            if (res.data) {
      ...
      loghis(cur){
        let url = '/api/loghis?id=' + this.$route.query.id + '&cur='
          + cur +'&type=0pname='+this.$route.query.pname +'&cname='
