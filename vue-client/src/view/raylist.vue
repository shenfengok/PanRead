  ...
  export default {
    ...
    methods :{
      ...
      getlist (is_next) {
        ...
        this.axios.get('/api/'+this.$route.query.prefix+'/list?id=' + this.$route.query.id + '&cur=' + this.cur)
          .then((res) => {
            if (res.data) {
              this.list = res.data
            } else {
      ...
      router2(subId){
        this.$router.push({ path: '/raydtl', query: {  id:this.$route.query.id,subId:subId,pname:this.$route.query.title,title:this.$route.query.title,prefix:this.$route.query.prefix }})
      },
      back(){
