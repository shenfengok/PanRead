...
const SYNC_ONE = '/api/book/queOne';
const SYNC_PARENT = '/api/book/queList';
...
const BUTTON_CONST = '<space>&nbsp;</space><button>one</button><space>&nbsp;</space><button >list</button>';
 ...
 * @returns {string}
 */
function get_base_path_for_item(it) {

    let a = $('.sharelist-history:first a').map(function(){
    ...
    do_for_element(BUTTON_GROUP_BUTTONS, x => {
        x.unbind('click').bind('click', async function () {
            ...
            if ('one' === type) {
                ...
                await syncOne(fsid,name,basePath);
            }
 ...
 * @returns {Promise<void>}
 */
async function syncOne(fsId, name, basePath) {
    let res = await post(SYNC_ONE, {fsId,name,basePath});
 ...
 * @returns {Promise<void>}
 */
async function syncParent(fsId, name, basePath) {
    let res = await post(SYNC_PARENT, {fsId,name,basePath});
    checkRes(res, '同步Parent内容');
    return true;
}
 ...
 * 添加buttion const
 */
async function addButtonConst() {

    //跳过条件，所有的job都有跳过条件
    //跳过条件
    if ($(BUTTON_GROUP).length === $(BUTTON_GROUP + '> button').length) {
        return;
    }
    ...
    do_for_element(BUTTON_GROUP, x => {
        x.each(function (index, el) {
            ...
            // let type = map[fsid];
            it.children('button').remove();
            it.children('space').remove();
            it.children('font').remove();
            // it.append('<font>(' + type + ')</font>');
            it.append(BUTTON_CONST);
            it.attr(BUTTON_FILLED_For, 'const');
        })
    })
