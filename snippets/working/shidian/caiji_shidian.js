...
function share_folder_2_pan_folder(name){
    return name.replace("/00-资源文件/04-十点读书/已经完结的课/001-050","").
    replace("/00-资源文件/04-十点读书/已经完结的课/100-","").
    replace("/00-资源文件/04-十点读书/已经完结的课/051-099","").
    replace("/00-资源文件/04-十点读书/更新中的课","");
}
...
async function caiji(){
    ...
    var list2 = await fetch_share_list("299761363283010");

    await sync_list(list2,big_obj,true);

    var list2 = await fetch_share_list("154377502367819");
