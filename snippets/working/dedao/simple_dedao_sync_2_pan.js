...
async function sync_folder_overwrite(fid){
    ...
    var file_data = {
        ...
        "path": "/apps/Cloud Sync/dedao-all/",
        "ondup": "overwrite",
        "async": "1",
        "type": "2",
...
async function del_folder(folder_name){
    ...
    var del_data= {
        filelist: "[\"/apps/Cloud Sync/dedao-all/"+folder_name+"\"]"
    };
...
function get_pan_list_url(folder){
    var dir = "%2Fapps%2FCloud+Sync%2Fdedao-all";
    if(folder){
        dir = "%2Fapps%2FCloud+Sync%2Fdedao-all%2F"+ encodeURI(folder);
    }
...
async function caiji(){
    ...
    var list3 = await fetch_share_list("185471743610487");

    await sync_list(list3);

    var list4 = await fetch_share_list("889869134963560");
