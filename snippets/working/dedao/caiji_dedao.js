...
function share_folder_2_pan_folder(name){
    return name.replace("/00-资源文件/02-得到/01-课程（已完结课程）/90元以上的课↑（已完结）","").
    replace("/00-资源文件/02-得到/01-课程（已完结课程）/官方下架的课（遗留课程）","").
    replace("/00-资源文件/02-得到/04-更新中的课","").
    replace("/00-资源文件/02-得到/01-课程（已完结课程）/90元以下的课↓（已完结）","");
}
...
async function caiji(){
    ...
    var list = await fetch_share_list("892227217147466");
    await sync_list(list,big_obj,true);

    var list2 = await fetch_share_list("770019881505174");

    await sync_list(list2,big_obj,true);

    var list2 = await fetch_share_list("185471743610487");

    await sync_list(list2,big_obj,true);

    var list2 = await fetch_share_list("889869134963560");
